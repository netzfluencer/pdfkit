...
import FacturXMixin from './mixins/facturX';
...
class PDFDocument extends stream.Readable {
  ...
  addNamedEmbeddedFile(name, ref, relationship) {
    if (!this._root.data.Names.data.EmbeddedFiles) {
      // disabling /Limits for this tree fixes attachments not showing in Adobe Reader
      this._root.data.Names.data.EmbeddedFiles = new PDFNameTree({ limits: false });
    }

    // If an AFRelationship is defined for the file to embed, than add the reference also to the AF Array
    if (relationship) {
      if (!this._root.data.AF || !Array.isArray(this._root.data.AF) ) {
        this._root.data.AF = []
      }
      this._root.data.AF.push(ref);
    }
